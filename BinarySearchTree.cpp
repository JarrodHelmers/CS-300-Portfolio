...
struct Node {
    Bid bid;      // Bid information stored in the node
    Node* left;   // Pointer to the left child node (values < parent)
    Node* right;  // Pointer to the right child node (values > parent)
    ...
    // Custom constructor with a bid parameter
    Node(Bid t_bid) : Node() { // Calls the default constructor
        this->bid = t_bid; // Assigns the bid to the node
    }
};
...
void BinarySearchTree::Insert(Bid bid) {
    if (root == nullptr) { // If the tree is empty
        ...
        this->addNode(root, bid); // Call the addNode function to insert the bid recursively
    }
}
...
Bid BinarySearchTree::Search(string bidId) {
    ...
    while (current != nullptr) {
        // If current node matches, return bid
        if (current->bid.bidId.compare(bidId) == 0) {
            return current->bid;
        }
        // If bid is smaller, move to the left subtree
...
void BinarySearchTree::inOrder(Node* node) {
    ...
    if (node != nullptr) { // If the node is not nullptr, perform the in-order traversal
...
// Traverse the right subtree after each level has been checked
        inOrder(node->right);
    }
}
...
// If the node is nullptr, return it
    if (node == nullptr) {
        return node;
    }
...
//remove from the left subtree
    if (bidId.compare(node->bid.bidId) < 0) {
        ...
        // If the current node has two children
            ...
            // Copy the bid from the smallest node to the current node and remove smallest node
            node->bid = temp->bid;
